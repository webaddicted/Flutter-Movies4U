...
class TrandingMovieRow extends StatelessWidget {
  ...
  final movieId;
  final sizeInfo;
  const TrandingMovieRow({super.key, required this.apiName,required this.sizeInfo, this.movieId = 0});
  ...
  Widget apiResponse(BuildContext context) {
    // printLog(tag: runtimeType.toString(),msg:"MovieId : $movieId");
      ...
      builder: (context, _, model) {
        ...
        if (jsonResult.status == ApiStatus.success) {
          ...
          return apiHandler(
              loading:  ShimmerView(
                  sizeInfo,
                  viewType: ShimmerView.viewHorizontalMovieList,
  ...
  Widget getMovieList(BuildContext context, String apiName, var jsonResult) {
    return Column(
      children: <Widget>[
        ...
        const SizedBox(height: 10),
        SizedBox(
          height:  sizeInfo.deviceScreenType == DeviceScreenType.desktop?380:240.0,
...
//              Result item = jsonResult[index];
  ...
  int getCount(jsonResult) {
    ...
    } else if (apiName == StringConst.personMovieCast &&
        jsonResult is PersonMovieRespo) {
      return jsonResult.cast!.length;
    } else if (apiName == StringConst.personMovieCrew &&
        jsonResult is PersonMovieRespo) {
      return jsonResult.crew!.length;
    }
    return 0;
  }
            ...
            SizedBox(
                    ...
                    child: ClipRRect(
                      ...
                      child: Stack(
                        children: [
                          getCacheImage(
                              url:img.contains("http") ? img : ApiConstant.imagePoster + img.toString(), height: height!),
                          Positioned.fill(
                              child: Material(
                                      ...
                                      onTap: () {
                                        if (onTap != null) {
                                          ...
                                          navigationPush(
                                              context,
                                              DetailsMovieScreen(
                                                  name, ApiConstant.imagePoster+img,apiName, index, id, tag));
                                        }
                                      }))),
                        ],
                      ),
...
//                      fontWeight: FontWeight.bold),
                  ...
                  RatingBar.builder(
                    ...
                    itemBuilder: (context, _) => Icon(
                      Icons.star,
                      color: getBackgroundRate(double.parse(vote.toString())),
                    ),
                    onRatingUpdate: (rating) {
                      printLog(tag: "Trading Movie Row",msg:"Rating : $rating");
                    },
                  )
                ],
              )
...
Color getBackgroundRate(double rate) {
  if (rate < 5.0) {
    return Colors.red;
  } else if (rate < 6.8) {
